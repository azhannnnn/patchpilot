...
#     state["auto_fix_data"]   = auto_fix
#     state["fingerprint"]     = fingerprint
#     state["agent_trace"]     = state.get("agent_trace", []) + [{
#         "agent": "MemoryAgent", "action": "Search incident memory (FAISS + Firestore)",
#         "reasoning": f"Fingerprint lookup + vector similarity for {alert.get('service','')}",
#         "tool_calls": [],
#         "output": f"{'REPEAT DETECTED — auto-fix eligible' if is_repeat else f'{len(similar)} similar incidents found'} (seen {repeat_count}×)",
#         "timestamp": datetime.utcnow().isoformat(),
#         "gemini_active": False
#     }]
#     _save_state(state["incident_id"], state)
...
#                 f"Seen {memory_agent.get_repeat_count(state.get('fingerprint',''))} times before",
#                 f"Previous resolution: {past_steps[0] if past_steps else 'known fix'}",
#             ],
#             "affected_services": [alert.get("service","unknown")],
#             "recommended_action": "Auto-fix applied from memory",
...
#         state["agent_trace"] = state.get("agent_trace",[]) + [{
#             "agent": "ControllerAgent", "action": "AUTO-FIX: Apply known remediation from memory",
#             "reasoning": "Repeat incident detected by MemoryAgent — applying previously successful fix automatically",
#             "tool_calls": [], "output": "Auto-fix applied — no human approval required. Incident closed.",
...
#                     "Incident closed automatically — no approval needed",
#                 ],
...
#             "reasoning": "Novel incident — remediation plan requires human authorisation before execution",
...
#     return "auto_fix" if (state.get("is_repeat_issue") and state.get("auto_fix_data")) else "full_pipeline"
